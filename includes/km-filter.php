 ...
 *  2. From name set in Kingmailer settings
 *  3. From `KINGMAILER_FROM_NAME` constant
 ...
 * @since	1.5.8
 */
function km_detect_from_name($from_name_header = null)
{
    // Get options to avoid strict mode problems
    $km_opts = get_option('kingmailer');
    $km_override_from = (isset($km_opts['override-from'])) ? $km_opts['override-from'] : null;
    $km_from_name = (isset($km_opts['from-name'])) ? $km_opts['from-name'] : null;

    $from_name = null;

    if ($km_override_from && !is_null($km_from_name)) {
        $from_name = $km_from_name;
    } elseif (!is_null($from_name_header)) {
        $from_name = $from_name_header;
    } elseif (defined('KINGMAILER_FROM_NAME') && KINGMAILER_FROM_NAME) {
        $from_name = KINGMAILER_FROM_NAME;
    } else {
        if (is_null($km_from_name) || empty($km_from_name)) {
 ...
 *  2. From address set in Kingmailer settings
 *  3. From `KINGMAILER_FROM_ADDRESS` constant
 ...
 * @since	1.5.8
 */
function km_detect_from_address($from_addr_header = null)
{
    // Get options to avoid strict mode problems
    $km_opts = get_option('kingmailer');
    $km_override_from = (isset($km_opts['override-from'])) ? $km_opts['override-from'] : null;
    $km_from_addr = (isset($km_opts['from-address'])) ? $km_opts['from-address'] : null;

    $from_addr = null;

    if ($km_override_from && !is_null($km_from_addr)) {
        $from_addr = $km_from_addr;
    } elseif (!is_null($from_addr_header)) {
        $from_addr = $from_addr_header;
    } elseif (defined('KINGMAILER_FROM_ADDRESS') && KINGMAILER_FROM_ADDRESS) {
        $from_addr = KINGMAILER_FROM_ADDRESS;
    } else {
        if (is_null($km_from_addr) || empty($km_from_addr)) {
            if (function_exists('get_current_site')) {
 ...
 * header like:
 *
 *  From: Excited User <user@samples.kingmailer.org>
 *
 * Will look like this in array format:
 *
 *  array(
 *      'from' => array(
 *          'value'    => 'Excited User <user@samples.kingmailer.org>',
 *          'boundary' => null,
 *          'parts'    => null,
 *      )
 *  )
 ...
 * @since	1.5.8
 */
function km_parse_headers($headers = array())
{
    if (empty($headers)) {
        return array();
    }
...
/**
 * Takes a header array in the format produced by km_parse_headers and
 ...
 * @since	1.5.8
 */
function km_dump_headers($headers = null)
{
