...
/**
 * km_smtp_last_error is a compound getter/setter for the last error that was
 * encountered during a Kingmailer SMTP conversation.
 *
 ...
 * @since 1.5.0
 */
function km_smtp_last_error($error = null)
{
    static $last_error;
...
function wp_mail_failed($error)
{
    if (is_wp_error($error)) {
        km_smtp_last_error($error->get_error_message());
    } else {
        km_smtp_last_error($error->__toString());
    }
}
 ...
 * @since 1.5.8
 */
function km_smtp_mail_filter(array $args)
{
    ...
    $km_headers = km_parse_headers($headers);

    // Filter the `From:` header
    $from_header = (isset($km_headers['From'])) ? $km_headers['From'][0] : null;
    ...
    $from_name = km_detect_from_name($from_name);
    $from_addr = km_detect_from_address($from_addr);

    $from_header['value'] = sprintf('%s <%s>', $from_name, $from_addr);
    $km_headers['From'] = array($from_header);

    // Header compaction
    $headers = km_dump_headers($km_headers);
